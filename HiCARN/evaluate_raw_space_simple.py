"""
Evaluate Predictions in RAW Space (No Normalization/Denormalization)

Input data is already in raw contact counts space.
Only performs shape conversion and metric calculation.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate predictions in RAW space (no normalization)')
    
    parser.add_argument('--pred_path', type=str, required=True,
                       help='Path to predictions in raw space')
    parser.add_argument('--gt_path', type=str, required=True,
                       help='Path to ground truth in raw space')
...
def main():
    ...
    print("="*80)
    print("EVALUATION IN RAW SPACE (NO NORMALIZATION)")
    print("="*80)
    
    # Load raw data (already in contact counts space)
    print(f"\nLoading predictions from: {args.pred_path}")
    pred_raw = np.load(args.pred_path)
    ...
    print(f"  Predictions range: [{pred_raw.min():.2f}, {pred_raw.max():.2f}]")
    print(f"  Ground truth range: [{gt_raw.min():.2f}, {gt_raw.max():.2f}]")
    
    # Convert to NCHW format
    print("\nConverting to NCHW format (N, 1, H, W)...")
    pred_nchw = convert_to_nchw(pred_raw)
    gt_nchw = convert_to_nchw(gt_raw)
