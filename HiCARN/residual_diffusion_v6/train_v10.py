#!/usr/bin/env python3
"""
Diffusion Refinement v10 - Peak Loss 诊断 + 自适应保护

================================================================================
v9 → v10 关键改进：

1. Peak Loss 诊断
   - 打印 peak_mask 覆盖率（topk_ratio 后多少像素被认为是 peak）
   - 打印 refined vs hicarn 在 peak 区域的 MSE
   - 打印 refined vs gt 在 peak 区域的 MSE
   - 如果 peak 区域完全没变，说明保护太死了

2. 自适应 Peak 保护
   - 新增 --peak_protection_mode: 'strict' / 'adaptive' / 'none'
   - strict: 原始行为，强保护峰值
   - adaptive: 允许模型在峰值区域做"向 GT 靠近"的更新
   - none: 不做峰值保护

3. 更详细的验证指标
   - peak_coverage: peak mask 覆盖的像素比例
   - peak_mse_vs_hicarn: refined 在 peak 区域相对 hicarn 的变化
   - peak_mse_vs_gt: refined 在 peak 区域相对 gt 的误差
   - peak_improvement: (hicarn_peak_mse - refined_peak_mse) / hicarn_peak_mse
...
# ================================================================
# Loss Functions with Peak Diagnostics
# ================================================================
...
def peak_preservation_loss(pred_matrix, gt_matrix, topk_ratio=0.02, mode='strict'):
    ...
    mode:
    - 'strict': 原始行为，惩罚 pred 与 gt 在 peak 区域的差异
    - 'adaptive': 只惩罚"远离 GT"的更新，允许"向 GT 靠近"的更新
    - 'none': 返回 0，不做峰值保护
    """
    if mode == 'none':
    ...
    if mode == 'strict':
        ...
        # 自适应：只惩罚"变得更差"的更新
        # 如果 |pred - gt| < |input - gt|，说明在改善，不惩罚
        # 这里我们没有 input (hicarn)，所以用不同的策略：
        # 只惩罚 pred 比 gt 小的情况（峰值被压低了）
        diff = F.relu(gt_matrix - pred_matrix)  # 只有当 pred < gt 时才有值
        diff_sq = diff ** 2
        peak_loss = (diff_sq * peak_mask).sum() / (peak_mask.sum() + 1e-8)
        return peak_loss
...
def peak_preservation_loss_with_diagnostics(pred_matrix, gt_matrix, hicarn_matrix, 
                                            topk_ratio=0.02, mode='strict'):
    """
    峰值保护 loss + 诊断信息
    
    返回:
    - loss: 峰值保护 loss
    - diagnostics: 包含各种诊断指标的字典
    """
    peak_mask, mask_diag = get_peak_mask_with_diagnostics(gt_matrix, topk_ratio)
    
    # 计算各种诊断指标
    with torch.no_grad():
        # Peak 区域：refined vs hicarn 的 MSE（模型在 peak 区域改变了多少）
        peak_change_vs_hicarn = ((pred_matrix - hicarn_matrix) ** 2 * peak_mask).sum() / (peak_mask.sum() + 1e-8)
        
        # Peak 区域：refined vs gt 的 MSE（模型在 peak 区域距离 GT 多远）
        peak_mse_vs_gt = ((pred_matrix - gt_matrix) ** 2 * peak_mask).sum() / (peak_mask.sum() + 1e-8)
        
        # Peak 区域：hicarn vs gt 的 MSE（baseline 在 peak 区域距离 GT 多远）
        hicarn_peak_mse_vs_gt = ((hicarn_matrix - gt_matrix) ** 2 * peak_mask).sum() / (peak_mask.sum() + 1e-8)
        
        # Peak 区域改善程度
        if hicarn_peak_mse_vs_gt > 1e-8:
            peak_improvement = (hicarn_peak_mse_vs_gt - peak_mse_vs_gt) / hicarn_peak_mse_vs_gt
        else:
            peak_improvement = torch.tensor(0.0)
        
        # 背景区域的类似指标
        bg_mask = 1 - peak_mask
        bg_change_vs_hicarn = ((pred_matrix - hicarn_matrix) ** 2 * bg_mask).sum() / (bg_mask.sum() + 1e-8)
        bg_mse_vs_gt = ((pred_matrix - gt_matrix) ** 2 * bg_mask).sum() / (bg_mask.sum() + 1e-8)
    
    # 计算 loss
    if mode == 'none':
        loss = torch.tensor(0.0, device=pred_matrix.device)
    elif mode == 'strict':
        diff = (pred_matrix - gt_matrix) ** 2
        loss = (diff * peak_mask).sum() / (peak_mask.sum() + 1e-8)
    elif mode == 'adaptive':
    ...
    diagnostics = {
        **mask_diag,
        'peak_change_vs_hicarn': peak_change_vs_hicarn.item(),
        'peak_mse_vs_gt': peak_mse_vs_gt.item(),
        'hicarn_peak_mse_vs_gt': hicarn_peak_mse_vs_gt.item(),
        'peak_improvement': peak_improvement.item() if torch.is_tensor(peak_improvement) else peak_improvement,
        'bg_change_vs_hicarn': bg_change_vs_hicarn.item(),
        'bg_mse_vs_gt': bg_mse_vs_gt.item(),
    }
...
# ================================================================
# Training with Peak Diagnostics
# ================================================================
...
def train_epoch_v10(
    ...
    insul_window=5,
    peak_protection_mode='strict',  # 新增: 'strict', 'adaptive', 'none'
    t_sampling='beta',
    t_p0=0.3,
    t_beta_a=0.5,
    t_beta_b=2.0,
):
    """v10 训练函数 - 带 peak diagnostics"""
    model.train()
    ...
    # Peak diagnostics
    peak_diag_accumulator = {
        'peak_change_vs_hicarn': 0,
        'peak_mse_vs_gt': 0,
        'hicarn_peak_mse_vs_gt': 0,
        'peak_improvement': 0,
        'bg_change_vs_hicarn': 0,
        'actual_coverage': 0,
    }
    ...
    for hicarn, gt in pbar:
        ...
        # Peak loss with diagnostics
        peak_loss, peak_diag = peak_preservation_loss_with_diagnostics(
            pred_final, gt, hicarn, 
            topk_ratio=gate_topk_ratio, 
            mode=peak_protection_mode
        )
        ...
        # Track peak diagnostics
        for k in peak_diag_accumulator:
            if k in peak_diag:
                peak_diag_accumulator[k] += peak_diag[k]
        ...
        pbar.set_postfix({
            'loss': f'{loss.item():.3f}',
            'corr': f'{corr:.3f}',
            'pk_imp': f'{peak_diag["peak_improvement"]:.3f}'
        })
    ...
    # Average peak diagnostics
    for k in peak_diag_accumulator:
        peak_diag_accumulator[k] /= num_batches
    metrics['peak_diagnostics'] = peak_diag_accumulator
...
def validate_v10(
    ...
    return {
        ...
        'peak_mse_hicarn_vs_gt': peak_mse_hicarn_vs_gt.item(),
        'peak_improvement': float(peak_improvement.item() if torch.is_tensor(peak_improvement) else peak_improvement),
        
        # 背景诊断
        'bg_mse_refined_vs_gt': bg_mse_refined_vs_gt.item(),
        'bg_mse_hicarn_vs_gt': bg_mse_hicarn_vs_gt.item(),
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='checkpoints_v10')
    ...
    # Peak protection mode (新增!)
    parser.add_argument('--peak_protection_mode', type=str, default='strict',
                       choices=['strict', 'adaptive', 'none'],
                       help='Peak protection mode: strict (original), adaptive (allow improvement), none (disable)')
    ...
    # ================================================================
    print("\n" + "="*80)
    print("DIFFUSION REFINEMENT v10 (with Peak Diagnostics)")
    print("="*80)
    print(f"\nv10 新增:")
    print(f"  1. Peak loss 诊断 - 打印 peak 区域的详细指标")
    print(f"  2. Peak protection mode: {args.peak_protection_mode}")
    print(f"     - strict: 强保护峰值（可能保护太死）")
    print(f"     - adaptive: 允许向 GT 靠近的更新")
    print(f"     - none: 不做峰值保护")
    print(f"  3. 详细的验证指标：peak_improvement, bg_improvement")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 5 == 0 or epoch == 1:
            ...
            # Peak diagnostics (关键新增!)
            print(f"    --- Peak Diagnostics ---")
            print(f"    peak_coverage: {val_metrics['peak_coverage']*100:.2f}% (topk_ratio={args.gate_topk_ratio})")
            print(f"    peak_mse (refined vs hicarn): {val_metrics['peak_mse_refined_vs_hicarn']:.6f}")
            print(f"    peak_mse (refined vs gt):     {val_metrics['peak_mse_refined_vs_gt']:.6f}")
            print(f"    peak_mse (hicarn vs gt):      {val_metrics['peak_mse_hicarn_vs_gt']:.6f}")
            print(f"    peak_improvement: {val_metrics['peak_improvement']*100:+.2f}%")
            print(f"    bg_improvement:   {val_metrics['bg_improvement']*100:+.2f}%")
            
            # 关键诊断：如果 peak 区域几乎没变，说明保护太死
            if val_metrics['peak_mse_refined_vs_hicarn'] < 1e-6:
                print(f"    ⚠️ WARNING: Peak region barely changed! Protection may be too strict.")
                print(f"       Try: --peak_protection_mode adaptive")
    ...
    if best_raw_res_corr < 0.10:
        print("⚠ raw_res_corr 未达标")
        print("  建议:")
        print("  1. 增大 warmup_epochs")
        print("  2. 尝试 --peak_protection_mode none (先学残差)")
    else:
        print("✓ raw_res_corr 达标!")
