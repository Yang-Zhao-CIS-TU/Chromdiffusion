...
Preserves the entire v15 residual-diffusion pipeline:
  ...
  distance-aware structure-proxy losses, spatial gate, alpha reconstruction.
...
class BottleneckSelfAttention(nn.Module):
    ...
    def __init__(self, channels, num_heads=4, init_scale=0.1):
        super().__init__()
        if channels % num_heads != 0:
            valid = [d for d in range(1, channels + 1) if channels % d == 0]
            raise ValueError(
                f"Bottleneck channels ({channels}) must be divisible by "
                f"--self_attn_heads ({num_heads}). Valid head counts: {valid}."
            )
        self.norm = nn.LayerNorm(channels)
        self.self_attn = nn.MultiheadAttention(
            embed_dim=channels, num_heads=num_heads, batch_first=True
        )
        self.self_attn_scale = nn.Parameter(torch.tensor(float(init_scale)))
    ...
    def forward(self, x):
        ...
        x_tokens = x_tokens + self.self_attn_scale * attn_out
...
class LightweightConditionEncoder(nn.Module):
    ...
    [B, C_bottleneck, H_bottleneck, W_bottleneck] with stride-2 convs.
    """

    def __init__(self, out_channels, num_downsamples=3):
        super().__init__()
        layers = []
        in_ch = 1
        for i in range(num_downsamples):
            remaining = num_downsamples - i
            out_ch = out_channels if remaining == 1 else max(out_channels // (2 ** (remaining - 1)), 1)
...
class BottleneckCrossAttention(nn.Module):
    ...
    def __init__(self, channels, num_heads=4, init_scale=0.1):
        ...
        if channels % num_heads != 0:
            ...
            raise ValueError(
                f"Bottleneck channels ({channels}) must be divisible by "
                f"--cross_attn_heads ({num_heads}). Valid head counts: {valid}."
            )
        ...
        self.cross_attn_scale = nn.Parameter(torch.tensor(float(init_scale)))

    def forward(self, x, cond_feat):
        B, C, H, W = x.shape
        if cond_feat.shape[-2:] != (H, W):
            cond_feat = F.interpolate(cond_feat, size=(H, W), mode='bilinear', align_corners=False)
        q_tokens = x.flatten(2).transpose(1, 2)          # [B, H*W, C]
        kv_tokens = cond_feat.flatten(2).transpose(1, 2)  # [B, H*W, C]
        q = self.norm_q(q_tokens)
        k = self.norm_kv(kv_tokens)
        v = k
        cross_out, _ = self.cross_attn(query=q, key=k, value=v, need_weights=False)
        q_tokens = q_tokens + self.cross_attn_scale * cross_out
        return q_tokens.transpose(1, 2).reshape(B, C, H, W)
...
class GatedConditionedUNet(nn.Module):
    def __init__(
        ...
        self_attn_heads=4,
        cross_attn_heads=4,
        self_attn_init_scale=0.1,
        cross_attn_init_scale=0.1,
    ):
        super().__init__()
        ...
        self.self_attn_init_scale = self_attn_init_scale
        self.cross_attn_init_scale = cross_attn_init_scale
        ...
        if use_cross_attention:
            ...
            self.bottleneck_cross_attn = BottleneckCrossAttention(
                bottleneck_ch, num_heads=cross_attn_heads, init_scale=cross_attn_init_scale
            )
        else:
            self.cond_encoder = None
            self.bottleneck_cross_attn = None
...
# ================================================================
# Tile Boundary Consistency Loss (formerly "stitch")
# ================================================================

def tile_boundary_consistency_loss(pred_batch, overlap_size=4):
    """
    Within-tile horizontal/vertical smoothness and boundary smoothness.

    This is a boundary-consistency proxy. It does not explicitly
    compare neighboring tiles because adjacent-tile pairs are not
    provided by the current training loader.

    Numerical behavior is identical to the former tile_stitch_consistency_loss().
    The argparse/config name --lambda_stitch is kept as a compatibility alias.
...
# Backward-compatible alias: same function object, same numerics.
tile_stitch_consistency_loss = tile_boundary_consistency_loss


def differentiable_pearson_loss(pred, target, eps=1e-8):
    """
    Fully differentiable Pearson correlation loss: mean(1 - corr).

    This replaces the previous non-differentiable SciPy diagnostic
    calculation (pred.detach().cpu().numpy() + scipy.stats.pearsonr
    wrapped in torch.tensor), which broke the computational graph
    and contributed zero gradient to pred_x0.

    Correlation is computed per sample over flattened spatial dims,
    then averaged across the batch. SciPy Pearsonr may still be used
    as a detached evaluation metric, never as this optimization loss.
    """
...
def train_epoch_v15(
    ...
    for batch_idx, batch_data in enumerate(pbar):
        ...
        # Direction loss: differentiable Pearson (replaces detached SciPy pearsonr)
        dir_loss = differentiable_pearson_loss(pred_x0, residual_scaled)
        ...
        # Proxy 3: within-tile boundary consistency (legacy name: stitch)
        boundary_loss = tile_boundary_consistency_loss(pred_final)
        stitch_loss = boundary_loss  # logging alias; same tensor
...
def validate_v15(
    ...
    stitch_loss = tile_boundary_consistency_loss(pred_final).item()
    ...
    metrics = {
        ...
        'grad_loss': grad_loss,
        'topk_loss': topk_loss,
        'stitch_loss': stitch_loss,  # legacy key; same value as boundary_consistency
        'boundary_consistency_loss': stitch_loss,
...
def build_checkpoint_config(args):
    ...
    cfg.update({
        ...
        'self_attn_heads': int(args.self_attn_heads),
        'cross_attn_heads': int(args.cross_attn_heads),
        'self_attn_init_scale': float(args.self_attn_init_scale),
        'cross_attn_init_scale': float(args.cross_attn_init_scale),
        ...
        'lambda_stitch': float(args.lambda_stitch),
...
def main():
    ...
    parser.add_argument('--no_self_attention', dest='use_self_attention', action='store_false',
                        help='Disable bottleneck self-attention (v15-equivalent if cross is also off).')
    parser.add_argument('--self_attn_heads', type=int, default=4)
    parser.add_argument('--self_attn_init_scale', type=float, default=0.1)
    parser.add_argument('--use_cross_attention', dest='use_cross_attention', action='store_true',
                        help='Enable bottleneck Y0 cross-attention (default).')
    parser.add_argument('--no_cross_attention', dest='use_cross_attention', action='store_false',
                        help='Disable bottleneck Y0 cross-attention.')
    parser.add_argument('--cross_attn_heads', type=int, default=4)
    parser.add_argument('--cross_attn_init_scale', type=float, default=0.1)
    parser.set_defaults(use_self_attention=True, use_cross_attention=True)
    ...
    parser.add_argument('--lambda_stitch', type=float, default=0.3,
                       help='Boundary-consistency proxy weight (legacy name: stitch).')
    parser.add_argument('--lambda_boundary', dest='lambda_stitch', type=float,
                       help='Alias of --lambda_stitch (preferred name).')
    ...
    # Resume / v15 warm-start
    parser.add_argument('--resume', type=str, default=None)
    parser.add_argument(
        '--init_from_v15', type=str, default=None,
        help='Load matching v15 weights (strict=False); new attention modules stay randomly initialized.'
    )
    ...
    print(f"Experiment: {args.experiment_name}  seed={args.seed}")
    ...
    model = GatedConditionedUNet(
        ...
        self_attn_init_scale=args.self_attn_init_scale,
        cross_attn_init_scale=args.cross_attn_init_scale,
    ).to(device)
    
    num_params = sum(p.numel() for p in model.parameters())
    print(f"Model parameters: {num_params / 1e6:.2f}M")
    print(f"  use_self_attention={args.use_self_attention}, use_cross_attention={args.use_cross_attention}")
    run_cfg = build_checkpoint_config(args)
    run_cfg['num_params'] = int(num_params)
    ...
    if args.resume and args.init_from_v15:
        raise ValueError("Use only one of --resume or --init_from_v15, not both.")

    if args.init_from_v15:
        print(f"\nInitializing from v15 checkpoint: {args.init_from_v15}")
        v15_ckpt = torch.load(args.init_from_v15, map_location=device)
        v15_state = v15_ckpt.get('model_state_dict', v15_ckpt)
        missing, unexpected = model.load_state_dict(v15_state, strict=False)
        print("Loaded v15 weights.")
        print("Missing keys correspond to newly added attention modules:")
        if missing:
            for k in missing:
                print(f"  {k}")
        else:
            print("  (none)")
        print("Unexpected keys in v15 checkpoint (not loaded into this model):")
        if unexpected:
            for k in unexpected:
                print(f"  {k}")
        else:
            print("  (none)")
        if 'res_mean' in v15_ckpt:
            print(f"  Note: residual/condition stats are recomputed from the current training data.")
        ...
        if epoch % 5 == 0 or epoch == 1:
            ...
            if val_metrics['pcc'] > best_pcc:
                ...
                torch.save({
                    ...
                    'best_pcc': best_pcc,
                    'best_iou': best_iou,
                    'config': build_checkpoint_config(args)
            ...
            print(f"    boundary_consistency: {val_metrics['stitch_loss']:.4f} (within-tile proxy; legacy stitch)")
