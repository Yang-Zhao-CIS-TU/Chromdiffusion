...
def main():
    ...
    for chrom in args.chromosomes:
        ...
        # Check for normalized GT in pred_dir first
        for pattern in [
        ...
        # Check for raw GT in gt_dir
        if gt_path is None and args.gt_dir:
            for pattern in [
        ...
        if gt_is_raw or gt_data.max() > 50:
            gt_raw = gt_data
            gt_norm = None  # Will skip norm evaluation
        ...
        # Denormalize to raw (same as test_v15.py: preproc.postprocess)
        base_raw = preproc.postprocess(base_norm)
        refined_raw = preproc.postprocess(refined_norm)
        
        print(f"  Base (raw): [{base_raw.min():.2f}, {base_raw.max():.2f}]")
        print(f"  Refined (raw): [{refined_raw.min():.2f}, {refined_raw.max():.2f}]")
        print(f"  GT (raw): [{gt_raw.min():.2f}, {gt_raw.max():.2f}]")
        
        # Save outputs (same naming as test_v15.py)
