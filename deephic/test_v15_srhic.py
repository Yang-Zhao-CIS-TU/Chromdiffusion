...
def load_preprocessor(path):
    ...
    if isinstance(data, dict):
        ...
        preprocessor.Y_mean = data.get('Y_mean', data.get('y_mean'))
        preprocessor.Y_std = data.get('Y_std', data.get('y_std'))
        preprocessor.X_mean = data.get('X_mean', data.get('x_mean'))
        preprocessor.X_std = data.get('X_std', data.get('x_std'))
    elif hasattr(data, 'Y_mean'):
        # Object format - copy attributes
        preprocessor.Y_mean = data.Y_mean
        preprocessor.Y_std = data.Y_std
        if hasattr(data, 'X_mean'):
            preprocessor.X_mean = data.X_mean
            preprocessor.X_std = data.X_std
    else:
        raise ValueError(f"Unknown preprocessor format: {type(data)}")
...
def main():
    ...
    for chrom in args.chromosomes:
        ...
        # Pattern 3: {hicarn_dir}/predictions_norm_{chrom}.npy
        # Pattern 4: {hicarn_dir}/predictions_{chrom}.npy
        hicarn_path = None
        for pattern in [
            ...
            os.path.join(args.hicarn_dir, f"predictions_{chrom}.npy"),
        ]:
        ...
        # Pattern 2: {hicarn_dir}/ground_truth_{chrom}.npy (normalized, same dir as HiCARN)
        # Pattern 3: {gt_dir}/hr_test_{chrom}.npy (raw, no ratio suffix)
        # Pattern 4: {gt_dir}/hr_test_{chrom}_ratio{ratio}.npy (raw)
        # Pattern 5: {gt_dir}/{chrom}/ground_truth.npy
        gt_path = None
        gt_is_raw = False
        
        for pattern, is_raw in [
            (os.path.join(args.hicarn_dir, chrom, "ground_truth.npy"), False),  # Normalized GT in subdir
            (os.path.join(args.hicarn_dir, f"ground_truth_{chrom}.npy"), False),  # Normalized GT same dir
            (os.path.join(args.gt_dir, f"hr_test_{chrom}.npy"), True),  # Raw GT no ratio
            (os.path.join(args.gt_dir, f"hr_test_{chrom}_ratio{args.ratio}.npy"), True),  # Raw GT with ratio
