...
def main():
    ...
    for chrom in args.chromosomes:
        ...
        for pattern, is_raw in [
            (os.path.join(args.gt_dir, f"hr_test_{chrom}_ratio{args.ratio}.npy"), True),  # Raw GT - CHECK FIRST!
            (os.path.join(args.gt_dir, f"hr_test_{chrom}.npy"), True),  # Raw GT without ratio
            (os.path.join(args.hicarn_dir, chrom, "ground_truth.npy"), False),  # Normalized GT fallback
        ]:
            if os.path.exists(pattern):
                gt_path = pattern
                gt_is_raw = is_raw
        ...
        # Denormalize using per-chromosome stats (if GT is raw) or global preprocessor (if GT is normalized)
        if gt_is_raw or gt_data.max() > 50:
            # GT is raw - use per-chromosome stats for fair comparison
            ...
            # Convert using per-chromosome stats
            hicarn_raw = np.expm1(np.clip(hicarn_norm, -5, 5) * chrom_std + chrom_mean)
            hicarn_raw = np.maximum(hicarn_raw, 0.0)
            refined_raw = np.expm1(np.clip(refined_norm, -5, 5) * chrom_std + chrom_mean)
            refined_raw = np.maximum(refined_raw, 0.0)
            
            print(f"    Using per-chrom stats: mean={chrom_mean:.4f}, std={chrom_std:.4f}")
        else:
            # GT is normalized - use global preprocessor
